...
const ExpertForm = (props) => {
  ...
  const uploadHandler = async (e) => {
    ...
    const uploadTask = storage
      .ref(
        `experts/${name}${work_location}${work_experience}${mobile}${email_address}.pdf`
      )
      .put(file)
    uploadTask.on(
      'state_changed',
      ...
      () => {
        storage
          .ref('experts')
          .child(
            `${name}${work_location}${work_experience}${mobile}${email_address}.pdf`
          )
          .getDownloadURL()
          .then((url) => {
            // console.log(url)
  ...
  return (
    <section className='grid grid-cols-1 lg:grid-cols-2' id='expert-form'>
      <div className='w-full px-4 sm:py-2 md:py-10 mx-auto xl:py-32 md:w-3/5 lg:w-4/5 xl:w-3/5'>
        <h1 className='mb-4 mt-4 text-2xl font-extrabold leading-snug tracking-tight text-left text-gray-900 md:text-4xl'>
          Join as "Empanelled Expert" today
        </h1>
        <form className='mt-8 space-y-4' onSubmit={uploadHandler}>
          ...
          <div className='col-span-6 sm:col-span-4'>
            ...
            <div
              className={`mt-1 flex justify-center px-6 pt-5 pb-6 border-2 p-2  border-dashed rounded-md ${
                file ? 'border-indigo-600' : 'border-gray-300'
              }`}
            >
        ...
        {progress === 0 ? (
          ...
          <div className='relative pt-1'>
            ...
            {progress === 100 && (
              <div
                className='p-4 flex items-center border-l-4 shadow-md border-indigo-600 fade show toast'
                role='alert'
                aria-live='assertive'
                aria-atomic='true'
              >
                <div className='toast-body'>
                  Success, we have recieved your details.
                </div>
                <button
                  type='button'
                  className='ml-auto mr-3 btn-close btn-close-white'
        ...
        {props.features.map((feature, index) => (
          <div className='flex space-x-3' key={index}>
            <svg
              ...
              viewBox='0 0 20 20'
              fill='currentColor'
              className='flex-none w-6 h-6 mt-1 text-indigo-600'
            >
              <path
                fillRule='evenodd'
